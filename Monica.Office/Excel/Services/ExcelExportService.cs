...
namespace Monica.Office.Excel.Services
{
    ...
    internal abstract class ExcelExportService : IExcelExporter
    {
            ...
            TaskProgress? taskProgress = null) where TExportDto : class
        {
            try
            {
                return ImplementExport(data, requests, optionAction, taskProgress);
            }
            catch (Exception e)
            {
        ...
        public Task<byte[]> ExportAsync<TExportDto>(IReadOnlyList<TExportDto> data, ExcelHeaderRequest[] requests, Action<ExcelExportOptions>? optionAction = null,
            TaskProgress? taskProgress = null) where TExportDto : class
        {
            return Task.FromResult(Export(data, requests, optionAction, taskProgress));
        }
            ...
            TaskProgress? taskProgress = null) where TExportDto : class
        {
            try
            {
                return Export(data,onlyExportHeaderName?.Select(p=> new ExcelHeaderRequest(p)).ToArray() ?? [], optionAction, taskProgress);
            }
            catch (Exception e)
            {
                taskProgress?.CancelTaskAsync($"导出失败: {e.Message}").Wait();
        ...
        public async Task<byte[]> ExportAsync<TExportDto>(IReadOnlyList<TExportDto> data,
            ...
            TaskProgress? taskProgress = null) where TExportDto : class
        {
            return await Task.FromResult(Export(data, optionAction, onlyExportHeaderName, taskProgress));
        }
