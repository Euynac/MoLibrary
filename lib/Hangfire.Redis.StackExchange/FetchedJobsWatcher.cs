...
{
    public class HangfireRedisGlobalOptions
    {
        public static HashSet<string> Queues { get; set; } = [];
        public static List<string> SupportedProject { get; set; } = [];
    }
...
#pragma warning restore 618
    {
        ...
        public void Execute(CancellationToken cancellationToken)
        {
            using (var connection = (RedisConnection)_storage.GetConnection())
            {
                ...
                {
                    if (!HangfireRedisGlobalOptions.Queues.Contains(queue.ToString())) continue;
                    ProcessQueue(queue.ToString(), connection);
                }
            }
