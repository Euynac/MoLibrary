...
| 03 | `03-monica-facade-skill-provider.md` | Auto-project every Monica module's Facade methods into a two-level Skill hierarchy (Module Skill → per-Facade Skills). |
...
graph TB
    ...
    subgraph Doc03["Doc 03 — Facade Skill Provider"]
        ModSkill[Module Skill aggregator]
        FacadeSkill[Per-Facade Skill]
    end
...
| **Facade Skill** | An `AgentSkill` auto-generated by the Facade Provider (Doc 03) — one per `IMonicaFacade`-marked type. Its scripts are the public methods of that Facade. |
| **Module Skill** | An `AgentSkill` auto-generated by the Facade Provider — one per Monica module that ships at least one Facade. Aggregates and points to the module's per-Facade Skills. Carries no scripts of its own. |
...
| `[MoAITool]` attribute | Method-or-parameter additive metadata. Two properties: `Description` (override) and `Disabled` (gate). On methods: optional description override AND on/off switch. On parameters: optional description override. Class-level usage forbidden by `AttributeUsage`. (Doc 02 §5.1) |
...
| `ModuleAIFacadeProvider` | `Monica.Framework` | Doc 03 | Auto-project `IMonicaFacade`-marked Facades into per-Facade and per-Module Skills. New BuiltInModuleKey: `AIFacadeProvider`. |
...
Startup phase (once)
    ...
    │       projects every IMonicaFacade type into a FacadeSkill,
    │       builds a ModuleAggregatorSkill per module
    │
...
| Hand-written module-specific Skills under a Module Skill umbrella | Doc 03 §3.5 — slot reserved in `ModuleAggregatorSkill` for future `ChildSkillNames`. | Future iteration when concrete use cases emerge. |
...
| `RAGFacade` semantics for already-configured pipelines | Out of scope — pipeline behavior unchanged; only KB-CRUD-shaped methods become `[Obsolete]` shim forwarders to `KnowledgeBaseFacade`. | — |
...
| 03 | Three end-to-end example tables (XML source → `AgentSkill` script → JSON tool schema). Module-Skill referencing-template example. Discovery sequence diagram. Method default deny-list table. |
