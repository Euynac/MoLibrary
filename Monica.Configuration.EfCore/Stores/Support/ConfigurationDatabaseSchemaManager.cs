        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _ = await dbContext.ConfigurationDefinitions
                .AsNoTracking()
                .OrderBy(static definition => definition.DefinitionKey)
        ...
        try
        {
            ...
            _ = await dbContext.ConfigurationValueHistories
                .AsNoTracking()
                .OrderBy(static history => history.HistoryId)
                .Select(history => new
                {
                    history.HistoryId,
            ...
            _ = await dbContext.ConfigurationEffectiveValues
                .AsNoTracking()
                .OrderBy(static value => value.DefinitionKey)
                .Select(value => new
                {
                    value.DefinitionKey,
            ...
            _ = await dbContext.ConfigurationUnifiedVersionDocuments
                ...
                .ThenBy(static document => document.DefinitionKey)
                .Select(document => new
                {
                    document.Version,
