...
/// <summary>
/// Implements the MoConfiguration Dashboard module
/// </summary>
public class ModuleConfigurationDashboard : MoModule
{
    private readonly ModuleConfigurationDashboardOption _option;

    /// <summary>
    /// Initializes a new instance of the <see cref="ModuleConfigurationDashboard"/> class
    /// </summary>
    /// <param name="option">Configuration options for the module</param>
    public ModuleConfigurationDashboard(ModuleConfigurationDashboardOption option)
    {
        _option = option;
    }
    ...
    public override Res ConfigureApplicationBuilder(IApplicationBuilder app)
    {
        if (_option.IsClient)
        {
            ...
            {
                var tagGroup = new List<Microsoft.OpenApi.Models.OpenApiTag> 
                { 
                    new() 
                    { 
                        Name = "MoConfiguration", 
                        Description = "热配置面板相关内置接口" 
                    } 
                };
    ...
    /// <summary>
    /// Returns the current module enumeration value
    /// </summary>
    /// <returns>The module enumeration value</returns>
    ...
    /// <summary>
    /// Configures the dashboard services
    /// </summary>
    /// <param name="services">The service collection to configure</param>
    private void ConfigureDashboard(IServiceCollection services)
    {
        Type dashboardType = _option.DashboardImplementationType ?? typeof(DefaultArrangeDashboard);
        
        services.AddSingleton(typeof(IMoConfigurationDashboard), dashboardType);
        ...
        Type storeType = _option.ConfigurationStoreType ?? typeof(MoConfigurationDefaultStore);
        services.AddSingleton(typeof(IMoConfigurationStores), storeType);
    ...
    private void ConfigureClient(IServiceCollection services)
    {
        ...
        if (_option.RegisterCentreServerConnectorType != null && _option.RegisterCentreClientType != null)
        {
            var serverConnectorType = _option.RegisterCentreServerConnectorType;
            var clientType = _option.RegisterCentreClientType;
            
            // Dynamically call AddMoRegisterCentreClient with provided types
            var method = typeof(RegisterCentreExtensions).GetMethod("AddMoRegisterCentreClient")
                ?.MakeGenericMethod(serverConnectorType, clientType);
...
/// <summary>
/// Request model for configuration rollback operations
/// </summary>
file class RollbackRequest
{
    /// <summary>
    /// The configuration key to rollback
    /// </summary>
    public required string Key { get; set; }
    
    /// <summary>
    /// The application ID associated with the configuration
    /// </summary>
