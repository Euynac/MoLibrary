...
namespace Hangfire.Redis
{
    internal class RedisConnection : JobStorageConnection
    {
        ...
        public override Dictionary<string, string>? GetAllEntriesFromHash([NotNull] string key)
        {
            ...
            if (result.TryGetValue(nameof(RecurringJobDto.Queue), out var queue) &&
                !HangfireRedisGlobalOptions.Queues.Contains(queue) && result.ContainsKey(nameof(RecurringJobDto.Job)))
            {
                return null;
            }
        ...
        {
            var jobId = Redis.SortedSetRangeByScore(_storage.GetRedisKey(key), fromScore, toScore, skip: 0, take: 1)
                .FirstOrDefault();
            if (jobId.HasValue && jobId.ToString() is {} jobIdStr)
            {
                if (HangfireRedisGlobalOptions.SupportedProject.Any(jobIdStr.Contains)) return jobIdStr;
            }
