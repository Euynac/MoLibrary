...
1. run `python scripts/validate_package.py --root .` and applicable source/localization checks
2. restore
3. build with warnings as errors
4. test
5. pack without rebuilding
6. inspect package contents and nuspec with `python scripts/inspect_package.py --root . --artifacts artifacts`
7. restore and run a clean consumer against a local feed
8. upload `.nupkg` and `.snupkg` as workflow artifacts
...
```bash
python scripts/validate_package.py --root . --package-version "${GITHUB_REF_NAME#v}"
```
