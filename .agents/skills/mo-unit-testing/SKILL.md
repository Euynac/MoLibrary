...
Create Monica unit tests under `tests/` using the shared `Test.Monica` base layer, `Test.Monica.*` naming, xUnit v3 for unit tests, and bUnit for Blazor UI tests. Follow Monica-specific rules for `Res<T>` assertions, module guide testing, and UI page or component boundaries instead of inventing per-project test conventions.
...
- Add reusable test helpers to `tests/Test.Monica/`.
...
- Shared test infrastructure project: `tests/Test.Monica/`
...
- Shared infrastructure: `tests/Test.Monica/`
