...
namespace Monica.DataChannel.Facades;

/// <summary>
/// Provides host-facing management operations for DataChannel.
/// </summary>
/// <remarks>
/// The primary constructor receives the dependencies required by the facade.
/// </remarks>
...
public class DataChannelFacade(IDataChannelManager manager, ILogger<DataChannelFacade> logger)
{
    ...
    public async Task<Res<List<DataChannelStatusInfo>>> GetChannelsStatusAsync()
    {
        try
        {
            var channels = manager.FetchAll().Select(channel => new DataChannelStatusInfo
            {
                Id = channel.Id,
                Middlewares = channel.Pipe.GetMiddlewares().Select(m => new PipelineComponentInfo(m)).ToList(),
                InnerEndpoint = new PipelineComponentInfo(channel.Pipe.InnerEndpoint),
                OuterEndpoint = new PipelineComponentInfo(channel.Pipe.OuterEndpoint),
    ...
    public async Task<Res<DataChannelExceptionInfo>> GetChannelExceptionsAsync(string id, int count = 10)
    {
        try
        {
            var channel = manager.Fetch(id);
            ...
            var result = new DataChannelExceptionInfo
            {
                ChannelId = id,
    ...
    public async Task<Res<DataChannelExceptionSummaryInfo>> GetExceptionSummaryAsync()
    {
        try
        {
            var channels = manager.FetchAll();
            
            var summary = new DataChannelExceptionSummaryInfo
            {
                ...
                ChannelSummaries = channels.Select(channel => new DataChannelExceptionSummaryItem
                {
