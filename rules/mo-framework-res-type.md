    ...
    CancellationToken cancellationToken)
{
    ...
    if (userInfo == null)
    {
        ...
        //or return Res.Fail($"用户名{request.Username}不存在");
        //不要使用泛型Res<string>.Fail("用户名不存在")
    }
...
```csharp
if ((await userManger.FillUserInfo(user)).IsFailed(out var error, out var data)) return error;
//use data...
```
