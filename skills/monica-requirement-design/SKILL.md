...
3. Analyze the current project structure:
   ...
   - Review relevant patterns from the `monica-development` skill references.
...
2. **Pattern selection**: Determine which Monica patterns apply:
   - Standard infrastructure module (Module, Option, required BuilderExtensions, optional RegistrationExtensions)
...
- Follow Monica module conventions (refer to `monica-development` skill for patterns).
