...
public class OptionItem
{
    ...
    public OptionItem(OptionSettingAttribute? info, PropertyInfo property, object? configInstance, string? parentKey)
    {
        ...
        if (info is null && ConfigurationRuntime.Setting.ErrorOnNoTagOptionAttribute)
        {
    ...
    /// </summary>
    public ConfigurationValueKind BasicType { get; set; }
    ...
    /// </summary>
    public ConfigurationDescriptor? SubConfigInfo { get; set; }
    ...
    public void SetValueFromConfigInstance(object? instance)
    {
        ...
        {
            if (type.GetGenericTypeDefinition() == typeof(Dictionary<,>) && type.GenericTypeArguments[1] is {IsClass:true} dictValueType && ConfigurationTypeAccessor.HasConfigAttribute(dictValueType))
            {
                SubConfigInfo = new ConfigurationDescriptor(dictValueType); // Nested configuration class inside dictionary value type.
            }
            else if (type.GetGenericTypeDefinition() == typeof(List<>) &&
                     type.GenericTypeArguments[0] is {IsClass: true} listValueType &&
                     ConfigurationTypeAccessor.HasConfigAttribute(listValueType))
            {
                SubConfigInfo = new ConfigurationDescriptor(listValueType); // Nested configuration class inside list item type.
            }
        }
        else if(PropertyInfo.PropertyType is {IsClass:true, IsArray:false} propertyType && propertyType != typeof(string))
        {
            SubConfigInfo = new ConfigurationDescriptor(propertyType); // Single nested configuration class.
        }
    }
    ...
    private string? GenValidateRegexPattern(PropertyInfo property)
    {
        ...
        if (BasicType == ConfigurationValueKind.Enum)
        {
    ...
    private void NormalizeType(Type type)
    {
        ...
        if (type.IsArray)
        {
            SpecialType = ConfigurationSpecialValueKind.Array;
        ...
        else if (type.IsGenericType)
        {
            ...
            {
                SpecialType = ConfigurationSpecialValueKind.Dict;
        ...
        if (UnderlyingType.IsValueType)
        {
            ...
            {
                BasicType = ConfigurationValueKind.Numeric;
                return;
            }
            if (DateTimeSet.Contains(UnderlyingType))
            {
                BasicType = ConfigurationValueKind.DateTime;
                return;
            }

            if (UnderlyingType.IsEnum)
            {
                BasicType = ConfigurationValueKind.Enum;
                return;
            }
            ...
            {
                BasicType = ConfigurationValueKind.Boolean;
                return;
            }

            if (UnderlyingType == typeof(TimeSpan))
            {
                BasicType = ConfigurationValueKind.TimeSpan;
                return;
            }
        }
        ...
        {
            BasicType = ConfigurationValueKind.Object;
            return;
        }

        BasicType = ConfigurationValueKind.String;
        return;
    }
