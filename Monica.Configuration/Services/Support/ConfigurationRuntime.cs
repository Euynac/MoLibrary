...
public static class ConfigurationRuntime
{
        ...
        get
        {
            if (_appConfiguration == null)
            {
                throw new InvalidOperationException(
                    $"AppConfiguration is not initialized in {typeof(ConfigurationRuntime)}.");
            }
    ...
    public static ModuleConfigurationOption Setting
    {
        get => _setting ?? throw new InvalidOperationException(
            $"Setting is not initialized in {typeof(ConfigurationRuntime)}. Please register ConfigurationDescriptor first");
        set => _setting = value;
    }
    ...
    public static List<ConfigurationProviderGroup> GetProvidersGrouped(this IConfiguration configuration)
    {
        var providers = ((IConfigurationRoot)configuration).Providers;
        var groupedProviders = new Dictionary<string, List<ConfigurationProviderSnapshot>>();
        ...
        foreach (var provider in providers)
        {
            ...
            var providerDto = new ConfigurationProviderSnapshot
            {
                Name = providerName,
                Type = providerType,
            ...
            {
                groupedProviders[providerType] = new List<ConfigurationProviderSnapshot>();
            }
            groupedProviders[providerType].Add(providerDto);
        }

        return groupedProviders.Select(group => new ConfigurationProviderGroup
        {
            GroupName = group.Key,
    ...
    /// </summary>
    /// <returns>A list of grouped configuration provider information.</returns>
    public static List<ConfigurationProviderGroup> GetProvidersGrouped() => GetProvidersGrouped(AppConfiguration);
