...
namespace Monica.Configuration.Extensions;
...
public static class ConfigurationLoggingExtensions
{
    ...
    {
        return new ConfigurationDescriptor(config, configInstance).ToString();
    }

    internal static string GetOptionItemString(Type config)
    {
        var configInstance = ConfigurationTypeAccessor.GetConfig(config);
    ...
    public static void LogCurrentConfig<T>(this T? config) where T : class
    {
        if (config == null)
        {
            ConfigurationRuntime.Logger.LogWarning("Option {FullName} is null.", typeof(T).GetCleanFullName());
            return;
        }

        var configInfo = ConfigurationDescriptor.Create(config);
        ...
        foreach (var option in items)
        {
            ...
            {
                ConfigurationRuntime.Logger.LogInformation("[{0}] {1}: {2}", typeof(T).Name, option.Name, displayValue);
            }
            else if (optionAttr.LoggingFormat is { } format)
            {
                ConfigurationRuntime.Logger.LogInformation(format, displayValue);
            }
            else if (optionAttr.Title is { } description)
            {
                ConfigurationRuntime.Logger.LogInformation("[{0}] {1}: {2}", typeof(T).Name, description, displayValue);
            }
        }
    }
}
