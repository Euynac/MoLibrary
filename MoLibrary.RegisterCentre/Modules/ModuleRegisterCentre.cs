...
public class ModuleRegisterCentre(ModuleRegisterCentreOption option) : MoModule<ModuleRegisterCentre, ModuleRegisterCentreOption>(option)
{
    ...
    public override Res ConfigureApplicationBuilder(IApplicationBuilder app)
    {
        if (option.ThisIsCentreServer)
        {
            ...
            {
                var tagGroup = new List<OpenApiTag> { new() { Name = option.SwaggerGroupName, Description = "注册中心" } };
        ...
        else if (option.ThisIsCentreClient)
        {
            app.UseEndpoints(endpoints =>
            {
                var tagGroup = new List<OpenApiTag> { new() { Name = option.SwaggerGroupName, Description = "注册中心客户端相关内置接口" } };
