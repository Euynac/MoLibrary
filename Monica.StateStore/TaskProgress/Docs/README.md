...
TaskProgress 模块提供了一个完整的进度条管理系统，支持状态存储、取消操作和事件通知。
...
// 创建进度条
var taskProgress = await _taskProgressService.CreateTaskProgressAsync("my-task", setting =>
{
...
await taskProgress.UpdateStatusAsync(50, "处理中...");

// 更新进度并设置阶段
await taskProgress.UpdateStatusAsync(50, "处理中...", "数据处理阶段");

// 递增进度
await taskProgress.IncrementAsync(1, "完成一个项目");

// 递增进度并更新阶段
await taskProgress.IncrementAsync(1, "完成一个项目", "文件处理阶段");

// 只更新阶段
await taskProgress.UpdatePhaseAsync("初始化阶段", "准备开始处理...");

// 强制立即保存状态
await taskProgress.SaveStatus(saveInstantly: true);
...
```csharp
// 获取基本状态
var status = await _taskProgressService.GetTaskProgressStatusAsync("my-task");
...
// 获取自定义状态
var customStatus = await _taskProgressService.GetTaskProgressStatusAsync<CustomTaskProgressStatus>("custom-task");
...
```csharp
// 完成任务
await taskProgress.CompleteTaskAsync();

// 取消任务
await taskProgress.CancelTaskAsync("用户取消");
```
...
```csharp
var taskProgress = await _taskProgressService.CreateTaskProgressAsync("auto-task", setting =>
{
    setting.AutoUpdateDuration = TimeSpan.FromSeconds(10); // 每10秒自动保存
});

// 这些调用不会立即保存（因为有自动更新）
await taskProgress.UpdateStatusAsync(10);
await taskProgress.UpdateStatusAsync(20);
...
{
    Console.WriteLine($"进度: {e.TaskProgress.Status.Percentage}%");
};
...
try
{
    var taskProgress = await _taskProgressService.CreateTaskProgressAsync("test");
    await taskProgress.UpdateStatusAsync(50);
}
catch (Exception ex)
{
    // 异常会包含详细的上下文信息
...
```csharp
// 定义自定义状态类
public class FileProcessingStatus : TaskProgressStatus
{
...
// 定义自定义进度条类
public class FileProcessingTaskProgress : TaskProgress
{
    public FileProcessingTaskProgress(TaskProgressSetting setting, ITaskProgressService service, string taskId)
...
// 使用自定义进度条
var fileTaskProgress = await _taskProgressService.CreateTaskProgressAsync<FileProcessingTaskProgress>("file-task");
await fileTaskProgress.ProcessFileAsync("document.pdf", 1024000);
```

### 阶段化进度跟踪

```csharp
var taskProgress = await _taskProgressService.CreateTaskProgressAsync("staged-task", setting =>
{
    setting.TotalSteps = 100;
});

// 第一阶段：初始化
await taskProgress.UpdatePhaseAsync("初始化", "正在初始化系统...");
for (int i = 1; i <= 20; i++)
{
    await taskProgress.UpdateStatusAsync(i, $"初始化步骤 {i}/20");
    await Task.Delay(100);
}

// 第二阶段：数据处理
await taskProgress.UpdatePhaseAsync("数据处理", "正在处理业务数据...");
for (int i = 21; i <= 80; i++)
{
    await taskProgress.UpdateStatusAsync(i, $"处理数据 {i-20}/60");
    await Task.Delay(50);
}

// 第三阶段：完成
await taskProgress.UpdatePhaseAsync("完成", "正在保存结果...");
for (int i = 81; i <= 100; i++)
{
    await taskProgress.UpdateStatusAsync(i, $"保存结果 {i-80}/20");
    await Task.Delay(30);
}
...
```csharp
var taskProgress = await _taskProgressService.CreateTaskProgressAsync("cancel-test");
...
taskProgress.Cancelled += (sender, e) =>
{
    Console.WriteLine($"任务被取消: {e.Reason}");
    // 状态中也会保存取消原因和取消标记
    Console.WriteLine($"状态中的取消原因: {e.TaskProgress.Status.CancelReason}");
    Console.WriteLine($"状态中的取消标记: {e.TaskProgress.Status.IsCancelled}");
};

try
{
    await taskProgress.UpdatePhaseAsync("处理中", "执行重要任务...");
    // ... 处理逻辑
}
catch (Exception ex)
{
    await taskProgress.CancelTaskAsync($"发生错误: {ex.Message}");
}

// 后续可以通过状态获取取消信息
var status = await _taskProgressService.GetTaskProgressStatusAsync("cancel-test");
if (status.IsCancelled)
{
...
// 在微服务B中获取状态
var taskStatus = await _taskProgressService.GetTaskProgressStatusAsync("cancel-test");
...
```csharp
// 获取基本状态
var basicStatus = await _taskProgressService.GetTaskProgressStatusAsync("my-task");

// 获取自定义状态
var customStatus = await _taskProgressService.GetTaskProgressStatusAsync<FileProcessingStatus>("file-task");
if (customStatus != null)
{
...
```csharp
// 好的做法：清晰的阶段划分
await taskProgress.UpdatePhaseAsync("数据验证", "验证输入数据格式...");
await taskProgress.UpdatePhaseAsync("数据转换", "转换数据格式...");
await taskProgress.UpdatePhaseAsync("数据保存", "保存到数据库...");

// 避免：过于细致的阶段划分
// await taskProgress.UpdatePhaseAsync("验证第1个字段", "...");
// await taskProgress.UpdatePhaseAsync("验证第2个字段", "...");
```
...
```csharp
// 好的做法：具体的取消原因
await taskProgress.CancelTaskAsync("用户手动取消操作");
await taskProgress.CancelTaskAsync("网络连接超时");
await taskProgress.CancelTaskAsync("磁盘空间不足");

// 避免：模糊的取消原因
// await taskProgress.CancelTaskAsync("出错了");
```
...
```csharp
// 好的做法：有业务意义的自定义属性
public class DataMigrationStatus : TaskProgressStatus
{
...
public class DataProcessingService
{
    ...
    {
        var taskProgress = await _taskProgressService.CreateTaskProgressAsync(taskId);
        
        try
        {
            await taskProgress.UpdatePhaseAsync("数据验证", "开始验证数据...");
            // 处理逻辑...
            
            if (someErrorCondition)
            {
                await taskProgress.CancelTaskAsync("数据验证失败");
                return;
            }
        }
        catch (Exception ex)
        {
            await taskProgress.CancelTaskAsync($"处理异常: {ex.Message}");
            throw;
        }
    }
}
...
public class NotificationService
{
    public async Task MonitorTaskAsync(string taskId)
    {
        while (true)
        {
            var status = await _taskProgressService.GetTaskProgressStatusAsync(taskId);
