...
namespace Monica.StateStore.TaskProgress.Samples;

/// <summary>
/// Demonstrates a custom task progress status model.
/// </summary>
public class CustomTaskProgressStatus(int totalSteps, string id) : TaskProgressStatus(totalSteps, id)
{
    /// <summary>
...
/// <summary>
/// Demonstrates a custom task progress model.
/// </summary>
public class CustomTaskProgress(TaskProgressSetting setting, ITaskProgressService service, string taskId)
    : TaskProgressModels.TaskProgress<CustomTaskProgressStatus>(setting, service, taskId)
{
    

    /// <summary>
    /// Updates progress while processing a file.
    /// </summary>
    ...
    public override CustomTaskProgressStatus CreateDefaultCustomStatus()
    {
        return new CustomTaskProgressStatus(Setting.TotalSteps, taskId);
    }
}
...
public class TaskProgressExample
{
    ...
    public TaskProgressExample(ITaskProgressService taskProgressService)
    {
    ...
    {
        var taskProgress = await _taskProgressService.CreateTaskProgressAsync("basic-example", setting =>
        {
        ...
        try
        {
            ...
            await taskProgress.UpdatePhaseAsync("初始化", "准备开始处理...");

            // Data loading phase
            await taskProgress.UpdatePhaseAsync("数据加载", "正在加载数据...");
            for (int i = 1; i <= 30; i++)
            {
                await taskProgress.UpdateStatusAsync(i, $"加载数据项 {i}/30");
                await Task.Delay(100); // 模拟处理时间
            }

            // Data processing stage
            await taskProgress.UpdatePhaseAsync("数据处理", "正在处理数据...");
            for (int i = 31; i <= 80; i++)
            {
                await taskProgress.UpdateStatusAsync(i, $"处理数据项 {i-30}/50");
        ...
        {
            await taskProgress.CancelTaskAsync("处理过程中发生错误");
            throw;
        }
    }
    ...
    {
        var taskProgress = await _taskProgressService.CreateTaskProgressAsync<CustomTaskProgress>("custom-example", setting =>
        {
        ...
        try
        {
            await taskProgress.UpdatePhaseAsync("文件处理", "开始处理文件...");

            // Simulate processing of multiple files
            var files = new[] { "file1.txt", "file2.txt", "file3.txt" };
            foreach (var file in files)
            {
                await taskProgress.UpdateFileProgressAsync(file, 1024 * 1024); // 1MB文件
        ...
        {
            await taskProgress.CancelTaskAsync("文件处理失败");
            throw;
        }
    }
    ...
    public async Task GetStatusExample()
    {
        ...
        // Get custom status
        var customStatus = await _taskProgressService.GetTaskProgressStatusAsync<CustomTaskProgressStatus>("custom-example");
    ...
    {
        var taskProgress = await _taskProgressService.CreateTaskProgressAsync("cancel-example");

        // Listen for cancellation events
        taskProgress.Cancelled += (_, e) =>
        {
        ...
        try
        {
            await taskProgress.UpdatePhaseAsync("处理中", "正在执行任务...");
            await Task.Delay(2000);

            // Simulate cancellation operation
            await taskProgress.CancelTaskAsync("用户主动取消");
        }
        ...
        var status = await _taskProgressService.GetTaskProgressStatusAsync("cancel-example");
        if (status == null)
        {
            throw new InvalidOperationException("Task progress 'cancel-example' was not found.");
        }
    ...
    public async Task CrossServiceStatusCheckExample()
    {
        ...
        var taskProgressA = await _taskProgressService.CreateTaskProgressAsync("cross-service-task", setting =>
        {
            setting.TotalSteps = 50;
        });

        // Start processing
        await taskProgressA.UpdatePhaseAsync("数据处理", "开始处理...");
        await taskProgressA.UpdateStatusAsync(10, "处理了10项");
        ...
        await taskProgressA.CancelTaskAsync("检测到异常，主动取消");
    ...
    private async Task<TaskProgressStatus> RequireStatusAsync(string id)
    {
        return await _taskProgressService.GetTaskProgressStatusAsync(id)
            ?? throw new InvalidOperationException($"Task progress '{id}' was not found.");
    }
} 
