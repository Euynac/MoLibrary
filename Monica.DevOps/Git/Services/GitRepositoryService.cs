    ...
    ILogger<GitRepositoryService> logger) : IGitRepositoryService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                var syncMessage = BuildSyncMessage(trigger, hasChanges, wasDirty);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                var deleteMessage = BuildDeleteMessage(pathExisted);

                state.CompleteDeletion(deleteMessage);
                await PublishDeletedEventAsync(registration.Id, resolvedLocalPath, pathExisted, cancellationToken);
    ...
    private static string BuildDeleteMessage(bool pathExisted)
    {
        return pathExisted
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning(ex, "Failed to publish Git repository deleted event for '{RepositoryId}'", repositoryId);
        }
    }
