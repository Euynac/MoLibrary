...
internal class RuntimeMetricsEventListener : EventListener
{
    ...
    private static (string counterName, double? counterValue) GetCounterData(IDictionary<string, object> eventPayload)
    {
        ...
        // 获取计数器名称
        if (eventPayload.TryGetValue("Name", out var nameValue)) counterName = nameValue.ToString();

        // 尝试获取计数器值（不同类型的计数器使用不同的字段）
        if (eventPayload.TryGetValue("Mean", out var meanValue))
        {
            if (double.TryParse(meanValue.ToString(), out var mean))
                counterValue = mean;
        }
        else if (eventPayload.TryGetValue("Increment", out var incrementValue))
        {
            if (double.TryParse(incrementValue.ToString(), out var increment))
                counterValue = increment;
        }
        else if (eventPayload.TryGetValue("Count", out var countValue))
        {
            if (double.TryParse(countValue.ToString(), out var count))
                counterValue = count;
        }
    ...
    {
        // 尝试获取CPU使用率计数器
        if (_counters.TryGetValue("cpu-usage", out var cpuUsage)) return cpuUsage;
    ...
    {
        // 优先返回工作集内存
        if (_counters.TryGetValue("working-set", out var workingSet)) return workingSet;
    ...
    public string GetMemoryInfo()
    {
        ...
        // 工作集内存
        if (_counters.TryGetValue("working-set", out var workingSet)) info.Add($"工作集: {FormatBytes(workingSet)}");

        // GC堆内存
        if (_counters.TryGetValue("gc-heap-size", out var gcHeapSize)) info.Add($"GC堆: {FormatBytes(gcHeapSize)}");

        // Gen 0 GC次数
        if (_counters.TryGetValue("gen-0-gc-count", out var gen0GcCount)) info.Add($"Gen0 GC: {gen0GcCount:F0}次");

        // Gen 1 GC次数
        if (_counters.TryGetValue("gen-1-gc-count", out var gen1GcCount)) info.Add($"Gen1 GC: {gen1GcCount:F0}次");

        // Gen 2 GC次数
        if (_counters.TryGetValue("gen-2-gc-count", out var gen2GcCount)) info.Add($"Gen2 GC: {gen2GcCount:F0}次");

        // 分配的内存
        if (_counters.TryGetValue("alloc-rate", out var allocRate)) info.Add($"分配速率: {FormatBytes(allocRate)}/s");

        // 活跃对象数量
        if (_counters.TryGetValue("gc-committed", out var gcCommitted)) info.Add($"GC已提交: {FormatBytes(gcCommitted)}");
