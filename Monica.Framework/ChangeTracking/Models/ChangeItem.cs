...
public class ChangeItem<TTargetEntity, TAlterItemData, TEnumAlterSource> : IChangeItem where TAlterItemData : class, IChangeItemData<TTargetEntity>, new() where TTargetEntity : class, IChangeTrackedEntity where TEnumAlterSource:Enum
{
    ...
    {
        DataJson = JsonSerializer.Serialize(_data, IChangeItem.ALTER_ITEM_SERIALIZER_OPTIONS);
    }
    ...
    /// <returns></returns>
    public List<ChangeRecord> GetChangesFromPreviousStatus(TTargetEntity previousStatus)
    {
        var list = new List<ChangeRecord>();
        if (TargetRollbackIds is not null)
        {
            list.AddIfNotNull(new ChangeRecord()
            {
        ...
        else
        {
            ...
            static ChangeRecord? Format(string displayName, object? oldValue, object? newValue)
            {
