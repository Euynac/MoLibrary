...
Skills install **project-first**: `configure --workspace <path>` installs the workspace's confirmed profile closure into the workspace-local directories configured in `.monica/guide.json` (`skillTargets`, default `.agents/skills`); the global catalogs (`~/.agents/skills`, `~/.claude/skills`) are explicit opt-ins. Initialized workspaces are registered in the engine registry (`state/workspaces.json`), and `guide workspaces` plus the wizard's Workspaces page list them all. Managed instruction rules carry stable catalog ids; `guide init`/`guide configure` accept `--rule <id>=on|off` to record per-workspace opt-outs as `disabledRules` in `.monica/guide.json` (rules default on) and converge the AGENTS.md block in the same run.
