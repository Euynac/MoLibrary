...
/// </summary>
internal sealed class FileDocumentIndexStateStore(
    IOptions<ModuleKnowledgeBaseOption> options,
    ILogger<FileDocumentIndexStateStore> logger) : IDocumentIndexStateStore
{
    ...
    public async Task<IReadOnlyList<KnowledgeBaseModel>> GetKnowledgeBasesAsync(CancellationToken ct = default)
    {
    ...
    public async Task<KnowledgeBaseModel?> GetKnowledgeBaseAsync(string knowledgeBaseId, CancellationToken ct = default)
    {
        await _lock.WaitAsync(ct);
        try
        {
    ...
    public async Task UpsertKnowledgeBaseAsync(KnowledgeBaseModel knowledgeBase, CancellationToken ct = default)
    {
        await _lock.WaitAsync(ct);
        try
        {
    ...
    public async Task<IReadOnlyList<DocumentIndexStateModel>> GetDocumentStatesAsync(
    ...
    public async Task<IReadOnlyList<DocumentIndexStateModel>> GetAllDocumentStatesAsync(CancellationToken ct = default)
    {
        await _lock.WaitAsync(ct);
        try
        {
    ...
    public async Task<DocumentIndexStateModel?> GetDocumentStateAsync(
        string knowledgeBaseId,
        string documentPath,
        CancellationToken ct = default)
    {
    ...
    public async Task UpsertDocumentStateAsync(DocumentIndexStateModel state, CancellationToken ct = default)
    {
        await _lock.WaitAsync(ct);
        try
        {
    ...
    private static KnowledgeBaseModel CloneKnowledgeBase(KnowledgeBaseModel kb)
        => kb with { };

    private static DocumentIndexStateModel CloneDocumentState(DocumentIndexStateModel state)
    {
        return new DocumentIndexStateModel
        {
    ...
    {
        public List<KnowledgeBaseModel> KnowledgeBases { get; init; } = [];

        public List<DocumentIndexStateModel> DocumentStates { get; init; } = [];
    }
}
